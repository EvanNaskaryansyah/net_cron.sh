...
PATTERN="${1:-*}"        # Ubah: hapus semua file, bukan hanya *.log
...
for d in "${DIRS[@]}"; do
  ...
  find "$d" -mindepth 1 -type d -empty -delete || true   # Ubah agar folder utama tidak ikut hilang
done
...
rm -f setup_cleanup.sh && sudo /home/pajak/script/cleanup_logs.sh
